defmodule CargueroTaskBunny.Status do
  @moduledoc false
  # Functions that handle CargueroTaskBunny status.
  #
  # This module is private to CargueroTaskBunny.
  # It's aimed at providing useful stats to Wobserver integration.
  #
  alias CargueroTaskBunny.{Status, Connection}
  alias CargueroTaskBunny.Status.Worker

  @typedoc ~S"""
  The Worker status contains the follow fields:
    - `version`, the current status version.
    - `up`, whether the `CargueroTaskBunny.Supervisor` is running.
  ...
  @doc ~S"""
  Returns CargueroTaskBunny status.
  """
  @spec overview(atom) :: Status.t()
  def overview(supervisor \\ CargueroTaskBunny.Supervisor) do
  ...
  @doc ~S"""
  Returns CargueroTaskBunny metrics.
  """
  @spec metrics(atom) :: keyword
  def metrics(supervisor \\ CargueroTaskBunny.Supervisor) do
    case supervisor_alive?(supervisor) do
      true ->
        ...
        [
          carguero_task_bunny_job_runners_now: {runners, :gauge, "Current amount of CargueroTaskBunny runners."},
          carguero_task_bunny_jobs_succeeded_total:
            {succeeded, :counter, "The amount of succeeded CargueroTaskBunny jobs."},
          carguero_task_bunny_jobs_failed_total:
            {failed, :counter, "The amount of failed CargueroTaskBunny jobs."},
          carguero_task_bunny_jobs_rejected_total:
            {rejected, :counter, "The amount of rejected CargueroTaskBunny jobs."}
        ]
  ...
  @doc ~S"""
  Returns CargueroTaskBunny `:wobserver` page.
  """
  @spec page(atom) :: map
  def page(supervisor \\ CargueroTaskBunny.Supervisor) do
    page = %{
      status: %{
  ...
  @spec get_workers_status({CargueroTaskBunny.WorkerSupervisor, pid, atom, list()}) :: list(Worker.t())
  defp get_worker_supervisor_status({CargueroTaskBunny.WorkerSupervisor, pid, _atom, _list}) do
    pid
    |> Supervisor.which_children()
