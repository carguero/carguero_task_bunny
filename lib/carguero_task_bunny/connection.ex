defmodule CargueroTaskBunny.Connection do
  @moduledoc """
  ...
  CargueroTaskBunny loads the configurations and automatically starts a GenServer for each host definition.
  They are supervised by CargueroTaskBunny so you don't have to look after them.

  ## Disconnect/Reconnect

  CargueroTaskBunny handles disconnection and reconnection.
  ...
  This will succeed in most cases since CargueroTaskBunny tries to establish a
  ...
  use GenServer
  require Logger
  alias CargueroTaskBunny.{Config, Connection.ConnectError}
  ...
  def start_link(state = {host, _, _}) do
    Logger.info("CargueroTaskBunny.Connection: start_link with #{host}")
  ...
  def handle_info(:connect, {host, _, listeners}) do
    case do_connect(host) do
      {:ok, connection} ->
        Logger.info("CargueroTaskBunny.Connection: connected to #{host}")
      ...
      error ->
        Logger.warn(
          "CargueroTaskBunny.Connection: failed to connect to #{host} - Error: #{inspect(error)}. Retrying in #{
            @reconnect_interval
          } ms"
        )
  ...
  def handle_info({:DOWN, _, :process, _pid, reason}, {host, _, _}) do
    Logger.warn("CargueroTaskBunny.Connection: disconnected from #{host} - PID: #{inspect(self())}")
  ...
  defp pname(host) do
    ("CargueroTaskBunny.Connection." <> Atom.to_string(host))
    |> String.to_atom()
  end
end
