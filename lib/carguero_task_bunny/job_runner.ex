defmodule CargueroTaskBunny.JobRunner do
  ...
  defp run_job(job, payload) do
    ...
    error ->
      Logger.debug("CargueroTaskBunny.JobRunner - Runner rescued #{inspect(error)}")
      {:error, JobError.handle_exception(job, payload, error)}
  catch
    _, reason ->
      Logger.debug("CargueroTaskBunny.JobRunner - Runner caught reason: #{inspect(reason)}")
