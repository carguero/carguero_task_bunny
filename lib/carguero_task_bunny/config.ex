defmodule CargueroTaskBunny.Config do
  @moduledoc """
  Handles CargueroTaskBunny configuration.
  """
  alias CargueroTaskBunny.ConfigError
  ...
  def auto_start? do
    case Application.fetch_env(:carguero_task_bunny, :disable_auto_start) do
      {:ok, true} -> false
      _ -> true
    end
  end
  ...
  def disable_worker? do
    case Application.fetch_env(:carguero_task_bunny, :disable_worker) do
      {:ok, true} -> true
  ...
  def disable_auto_start do
    :ok = Application.put_env(:carguero_task_bunny, :disable_auto_start, true)
  end

  @spec hosts_config() :: list
  defp hosts_config do
    case Application.fetch_env(:carguero_task_bunny, :hosts) do
  ...
  It returns `CargueroTaskBunny.FailureBackend.Logger` by default.
  """
  @spec failure_backend :: [atom]
  def failure_backend do
    case Application.fetch_env(:carguero_task_bunny, :failure_backend) do
      {:ok, list} when is_list(list) -> list
      {:ok, atom} when is_atom(atom) -> [atom]
      _ -> [CargueroTaskBunny.FailureBackend.Logger]
    end
  end
  ...
  def publisher_pool_size, do: Application.get_env(:carguero_task_bunny, :publisher_pool_size, 15)

  @doc """
  Returns the max overflow for the publisher poolboy. 0 by default
  """
  @spec publisher_max_overflow :: integer
  def publisher_max_overflow, do: Application.get_env(:carguero_task_bunny, :publisher_max_overflow, 0)
end
