defmodule CargueroTaskBunny.Worker do
  @moduledoc """
  A GenServer that listens a queue and consumes messages.

  You don't have to call or start worker explicity.
  CargueroTaskBunny loads config and starts workers automatically for you.
  ...
  @doc false
  @spec terminate(any, CargueroTaskBunny.Worker.t()) :: :normal
  ...
  def handle_call(:status, _from, state) do
    ...
    status = %CargueroTaskBunny.Status.Worker{
      queue: state.queue,
  ...
  defp pname(queue) do
    String.to_atom("CargueroTaskBunny.Worker.#{queue}")
  end
  ...
  defp log_msg(message, state, additional \\ nil) do
    message =
      "CargueroTaskBunny.Worker: #{message}. Queue: #{state.queue}. Concurrency: #{state.concurrency}. PID: #{
        inspect(self())
      }."
