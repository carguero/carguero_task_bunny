defmodule CargueroTaskBunny.Job do
  ...
  CargueroTaskBunny job is an asynchronous background job whose execution request is
  ...
  By default CargueroTaskBunny terminates the job when it takes more than 2 minutes.
  ...
  By default CargueroTaskBunny retries 10 times every five minutes for a failed job.
  ...
  CargueroTaskBunny will set failed count to the argument.
  The value will be more than or equal to 1 and less than or equal to max_retry.
  """
  @callback retry_interval(integer) :: integer

  require Logger
  alias CargueroTaskBunny.{Config, Queue, Job, Message, Publisher}

  alias CargueroTaskBunny.{
  ...
  defmacro __using__(_options \\ []) do
    quote do
      ...
      def enqueue(payload, options \\ []) do
        CargueroTaskBunny.Job.enqueue(__MODULE__, payload, options)
      end
